...
namespace Utils
{
    ...
    public class Vertices : List<FVector2>
    {
        ...
        public Vertices(FVector2[] vector2)
        {
        ...
        public Vertices(IList<FVector2> vertices)
        {
        ...
        public FVector2 GetCentroid()
        {
            ...
            FVector2 c = FVector2.Zero;
            float area = 0.0f;

            const float inv3 = 1.0f / 3.0f;
            FVector2 pRef = FVector2.Zero;
            for (int i = 0; i < Count; ++i)
            {
                // Triangle vertices.
                FVector2 p1 = pRef;
                FVector2 p2 = this[i];
                FVector2 p3 = i + 1 < Count ? this[i + 1] : this[0];
        ...
        /// </summary>
        /// <returns></returns>
        public Box2D.AABB GetCollisionBox()
        {
            Box2D.AABB aabb;
            FVector2 lowerBound = new FVector2(float.MaxValue, float.MaxValue);
            FVector2 upperBound = new FVector2(float.MinValue, float.MinValue);
        ...
        public void Translate(FVector2 vector)
        {
        ...
        {
            for (int i = 0; i < Count; i++)
                this[i] = FVector2.Add(this[i], vector);
        }
        ...
        {
            for (int i = 0; i < Count; i++)
                this[i] = FVector2.Multiply(this[i], value);
        }
        ...
        public bool IsSimple2()
        {
            for (int i = 0; i < Count; ++i)
            {
                ...
                int j = (i + 1) % Count;
                FVector2 iToj = this[j] - this[i];
                FVector2 iTojNormal = new FVector2(iToj.Y, -iToj.X);
                ...
                int k = startK;
                FVector2 iTok = this[k] - this[i];
                bool onLeftSide = FVector2.Dot(iTok, iTojNormal) >= 0;
                FVector2 prevK = this[k];
                ++k;
                for (; k <= endK; ++k)
                {
                    int modK = k % Count;
                    iTok = this[modK] - this[i];
                    if (onLeftSide != FVector2.Dot(iTok, iTojNormal) >= 0)
                    {
                        FVector2 prevKtoK = this[modK] - prevK;
                        FVector2 prevKtoKNormal = new FVector2(prevKtoK.Y, -prevKtoK.X);
                        if ((FVector2.Dot(this[i] - prevK, prevKtoKNormal) >= 0) !=
                            (FVector2.Dot(this[j] - prevK, prevKtoKNormal) >= 0))
                        {
                            return true;
                        }
                    }
                    onLeftSide = FVector2.Dot(iTok, iTojNormal) > 0;
                    prevK = this[modK];
                }
            }
            return false;
        ...
        public bool CheckPolygon(out int error, out string errorMessage)
        {
            ...
            if (Count < 3 || Count > Box2D.Settings.b2_maxPolygonVertices)
            {
                error = 0;
            }
            if (!IsConvex())
            ...
            if (GetArea() < Box2D.Settings.b2_epsilon)
            {
                error = 3;
            }

            //Compute normals
            FVector2[] normals = new FVector2[Count];
            Vertices vertices = new Vertices(Count);
            for (int i = 0; i < Count; ++i)
            {
                vertices.Add(new FVector2(this[i].X, this[i].Y));
                int i1 = i;
                int i2 = i + 1 < Count ? i + 1 : 0;
                FVector2 edge = new FVector2(this[i2].X - this[i1].X, this[i2].Y - this[i1].Y);
            ...
            for (int i = 0; i < Count; ++i)
            {
                ...
                if (angle <= Box2D.Settings.b2_angularSlop)
                {
                    error = 4;
                    break;
                }
                ...
                for (int j = 0; j < Count; ++j)
                {
                    ...
                    float s = FVector2.Dot(normals[i], vertices[j] - vertices[i]);
                    if (s >= -Box2D.Settings.b2_linearSlop)
                    {
                        error = 5;
                    }
                }
                ...
                FVector2 centroid = vertices.GetCentroid();
                FVector2 n1 = normals[iminus];
                FVector2 n2 = normals[i];
                FVector2 v = vertices[i] - centroid;

                FVector2 d = new FVector2();
                d.X = FVector2.Dot(n1, v); // - toiSlop;
                d.Y = FVector2.Dot(n2, v); // - toiSlop;
                ...
                switch (error)
                {
                    case 0:
                        errorMessage = string.Format("Polygon error: must have between 3 and {0} vertices.",
                            Box2D.Settings.b2_maxPolygonVertices);
                        break;
                    case 1:
        ...
        public List<Vertices> SplitAtHoles()
        {
            ...
            List<FVector2> duplicate = new List<FVector2>();
            int holeCount = 0;
            int index = 0;
        ...
        private class PolyNode
        {
            ...
            public PolyNode GetRightestConnection(FVector2 incomingDir)
            {
        ...
        public void ProjectToAxis(ref FVector2 axis, out float min, out float max)
        {
            // To project a point on an axis use the dot product
            float dotProduct = FVector2.Dot(axis, this[0]);
            min = dotProduct;
            max = dotProduct;
        ...
        public int PointInPolygon(ref FVector2 point)
        {
            ...
            for (int i = 0; i < Count; i++)
            {
                // Get points
                FVector2 p1 = this[i];
                FVector2 p2 = this[NextIndex(i)];

                // Test if a point is directly on the edge
                FVector2 edge = p2 - p1;
                float area = MathUtils.Area(ref p1, ref p2, ref point);
                if (area == 0f && FVector2.Dot(point - p1, edge) >= 0f && FVector2.Dot(point - p2, edge) <= 0f)
                {
                    return 0;
                }
        ...
        public bool PointInPolygonAngle(ref FVector2 point)
        {
            ...
            {
                // Get points
                FVector2 p1 = this[i] - point;
                FVector2 p2 = this[NextIndex(i)] - point;
